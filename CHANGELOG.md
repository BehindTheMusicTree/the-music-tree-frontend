...
- **Header**: larger nav links (**`text-base`**), **Contribute** CTA (**`text-sm`**, padding), lockup image height **48px**, and nav vertical padding (**`py-5`**). Below **`md`**, the header shows the **mark** only (**`the-music-tree-mark.svg`**); from **`md`** up, the full **TheMusicTree** horizontal lockup is shown (so **`sm`** viewports use the mark, not the lockup). **`HeaderTheMusicTreeBrand`** replaces **`HeaderTheMusicTreeLockup`** (name: not always a lockup). Below **`sm`**, primary nav and **Contribute** sit in a **menu** (toggle button, panel under the bar, **Escape** / resize to **`sm`** closes it); from **`sm`** up, links stay in the horizontal bar.
