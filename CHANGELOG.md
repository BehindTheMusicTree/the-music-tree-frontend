...
- **`/contact`**: channel layout matches **`/contribute`** “where to start” card (**bordered** outer card, **`md`** two-column grid, centered cells); **GitHub issues**, **discussions**, and **Discord** each have a pill + blurb; **Mastodon**, **LinkedIn**, **X**, and **email** share one grid cell; **newsletter** inner card uses shared **`NewsletterSignupInCard`** (**`useTranslations('newsletter')`**, same **`contributeSectionTitle`** / **`contributeSectionLead`** as **`/contribute`**). Page prose lives in **`messages.contact`** (**EN**/**FR**).
